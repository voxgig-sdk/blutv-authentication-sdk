...
//     (\`new BlutvAuthenticationSDK(...)\` / \`BlutvAuthenticationSDK.test(...)\` become
//     \`BlutvAuthenticationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'BlutvAuthenticationSDK'
...
const TEST_SEED = {"entity":{"login":{"test01":{"id":"test01"}},"password_recovery":{"test01":{"id":"test01"}},"register":{"test01":{"id":"test01"}},"social_login":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BlutvAuthenticationSDK(...)\` and any
// \`BlutvAuthenticationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BlutvAuthenticationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BlutvAuthenticationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
