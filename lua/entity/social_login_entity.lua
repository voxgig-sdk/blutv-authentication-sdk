...
local SocialLoginEntity = {}
SocialLoginEntity.__index = SocialLoginEntity


function SocialLoginEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SocialLoginEntity)
  self._name = "social_login"
  self._client = client
...
function SocialLoginEntity:get_name()
  return self._name
end


function SocialLoginEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SocialLoginEntity.new(self._client, opts)
end
...
function SocialLoginEntity:mark_deleted()
  self._deleted = true
end


function SocialLoginEntity:deleted()
  return true == self._deleted
end


function SocialLoginEntity:data_set(args)
  if args ~= nil then
...
function SocialLoginEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SocialLoginEntity:match_set(args)
  if args ~= nil then
...
function SocialLoginEntity:match_get()
...
function SocialLoginEntity:stream(action, args, callopts)
...
function SocialLoginEntity:_run_op(ctx, post_done)
  local utility = self._utility
