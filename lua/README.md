...
The Lua SDK for the BlutvAuthentication API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:Login(nil):create({ name = "Example" }, nil)
...
local result, err = client:BlutvAuthentication(nil):load(
  { id = "test01" }, nil
)
