...
require("blutv-authentication_types")
...
local BlutvAuthenticationSDK = {}
BlutvAuthenticationSDK.__index = BlutvAuthenticationSDK
...
BlutvAuthenticationSDK._make_feature = _make_feature


function BlutvAuthenticationSDK.new(options)
  local self = setmetatable({}, BlutvAuthenticationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BlutvAuthenticationSDK:options_map()
...
function BlutvAuthenticationSDK:get_utility()
  return Utility.copy(self._utility)
end


function BlutvAuthenticationSDK:get_root_ctx()
  return self._rootctx
end


function BlutvAuthenticationSDK:prepare(fetchargs)
...
function BlutvAuthenticationSDK:direct(fetchargs)
...
function BlutvAuthenticationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function BlutvAuthenticationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "BlutvAuthenticationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function BlutvAuthenticationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BlutvAuthenticationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BlutvAuthenticationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BlutvAuthenticationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BlutvAuthenticationSDK.new(sdkopts)
  sdk.mode = "test"
