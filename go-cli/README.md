# blutv-authentication-cli

boru-driven command-line client **and** interactive REPL for the BlutvAuthentication
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/blutv-authentication-cli)
make build

# 2. See usage (words, entities, env vars)
./blutv-authentication-cli --help
...
BLUTV_AUTHENTICATION_BASE=https://api.example.com ./blutv-authentication-cli --help

# 6. No arguments -> interactive REPL
./blutv-authentication-cli
blutv-authentication> /help
blutv-authentication> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/blutv-authentication-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/blutv-authentication-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `blutv-authentication>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `login`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/blutv-authentication-cli`. |
...
(`./blutv-authentication-cli <expr>`) and interactively (the REPL), and expressions compose the
