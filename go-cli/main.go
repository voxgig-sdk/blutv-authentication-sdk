// Command BlutvAuthentication-cli is a CLI + REPL for the BlutvAuthentication SDK,
...
const prompt = "blutv-authentication"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "login password_recovery register social_login"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BLUTV_AUTHENTICATION_APIKEY (api key), BLUTV_AUTHENTICATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BLUTV_AUTHENTICATION_APIKEY carries the API key and
	// BLUTV_AUTHENTICATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BLUTV_AUTHENTICATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BLUTV_AUTHENTICATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBlutvAuthenticationSDK(opts)
