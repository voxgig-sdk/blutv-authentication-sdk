...
// BlutvAuthentication SDK retry feature
...
class BlutvAuthenticationRetryFeature extends BlutvAuthenticationBaseFeature
{
    ...
    public function init(BlutvAuthenticationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BlutvAuthenticationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BlutvAuthenticationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BlutvAuthenticationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
