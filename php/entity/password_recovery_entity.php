...
// BlutvAuthentication SDK PasswordRecovery entity
...
class PasswordRecoveryEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PasswordRecoveryEntity($this->_client, $opts);
    }

    /**
     * @param PasswordRecovery|array $args PasswordRecovery data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BlutvAuthenticationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PasswordRecovery|array The current PasswordRecovery data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PasswordRecovery fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BlutvAuthenticationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PasswordRecovery fields).
     */
     ...
     *
     * @param PasswordRecoveryCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed PasswordRecoveryCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PasswordRecovery|array The created PasswordRecovery as an assoc-array at the
     *   SDK boundary; throws BlutvAuthenticationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = BlutvAuthenticationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
