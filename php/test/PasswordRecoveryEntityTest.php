...
class PasswordRecoveryEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUTV_AUTHENTICATION_TEST_PASSWORD_RECOVERY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $password_recovery_ref01_data = Helpers::to_map(is_object($password_recovery_ref01_data_result) && method_exists($password_recovery_ref01_data_result, 'data_get') ? $password_recovery_ref01_data_result->data_get() : $password_recovery_ref01_data_result);
...
function password_recovery_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUTV_AUTHENTICATION_TEST_PASSWORD_RECOVERY_ENTID");
