...
// BlutvAuthentication SDK feature test
//
...
class FtCtrl extends BlutvAuthenticationControl
{
...
class FtClient
{
    ...
    private BlutvAuthenticationUtility $utility;

    public function __construct(BlutvAuthenticationUtility $utility, array $options)
    {
    ...
    public function get_utility(): BlutvAuthenticationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BlutvAuthenticationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BlutvAuthenticationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BlutvAuthenticationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BlutvAuthenticationConfig::make_feature($name);
    ...
    private static function buildUrl(BlutvAuthenticationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BlutvAuthenticationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BlutvAuthenticationResponse($fetched);
        }
    ...
    private function fail(BlutvAuthenticationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BlutvAuthenticationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
