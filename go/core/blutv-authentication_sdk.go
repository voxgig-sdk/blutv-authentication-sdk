...
type BlutvAuthenticationSDK struct {
	Mode     string
...
func (sdk *BlutvAuthenticationSDK) OptionsMap() map[string]any {
...
func (sdk *BlutvAuthenticationSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *BlutvAuthenticationSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *BlutvAuthenticationSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *BlutvAuthenticationSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *BlutvAuthenticationSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *BlutvAuthenticationSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("BlutvAuthenticationSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *BlutvAuthenticationSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *BlutvAuthenticationSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("BlutvAuthenticationSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *BlutvAuthenticationSDK {
	...
	sdk := NewBlutvAuthenticationSDK(sdkopts)
	sdk.Mode = "test"
