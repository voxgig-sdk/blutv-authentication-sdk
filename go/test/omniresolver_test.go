...
import (
	...
	omni "github.com/voxgig-sdk/blutv-authentication-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BlutvAuthenticationError); is {
				out := map[string]any{
					"name":    "BlutvAuthenticationError",
...
func omniCtx(arg any, client *sdk.BlutvAuthenticationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BlutvAuthenticationSDK); is {
			client = live
