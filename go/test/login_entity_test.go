...
func TestLoginEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUTVAUTHENTICATION_TEST_LOGIN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func loginBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUTVAUTHENTICATION_TEST_LOGIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUTVAUTHENTICATION_TEST_LOGIN_ENTID"])
