...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUTVAUTHENTICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUTVAUTHENTICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUTVAUTHENTICATION_TEST_EXPLAIN"); explain != "" {
