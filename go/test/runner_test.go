...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUTV_AUTHENTICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUTV_AUTHENTICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUTV_AUTHENTICATION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.BlutvAuthenticationSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.BlutvAuthenticationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BlutvAuthenticationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.BlutvAuthenticationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.BlutvAuthenticationError{Msg: msg, Code: code}
}
