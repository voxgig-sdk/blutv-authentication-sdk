...
module BlutvAuthenticationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BlutvAuthenticationResponse.new({})
    ...
    if fetch_err
      ...
      response = BlutvAuthenticationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BlutvAuthenticationResponse.new(fetched)
    else
