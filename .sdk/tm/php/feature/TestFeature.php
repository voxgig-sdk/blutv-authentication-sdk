...
// BlutvAuthentication SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BlutvAuthenticationTestFeature extends BlutvAuthenticationBaseFeature
{
    ...
    public function init(BlutvAuthenticationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BlutvAuthenticationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BlutvAuthenticationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BlutvAuthenticationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BlutvAuthenticationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BlutvAuthenticationParam::call($ctx, $k);
