...
// BlutvAuthentication SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = BlutvAuthenticationOmni::makeStructRunner(
    ...
    {
        return BlutvAuthenticationOmni::objmodel(BlutvAuthenticationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return BlutvAuthenticationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return BlutvAuthenticationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(BlutvAuthenticationOmni::structfix($now, Struct::undef()))
                        !== json_encode(BlutvAuthenticationOmni::structfix($val, Struct::undef()))) {
