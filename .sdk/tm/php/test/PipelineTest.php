...
// BlutvAuthentication SDK pipeline test
//
...
{
    private static function utility(): BlutvAuthenticationUtility
    {
        return new BlutvAuthenticationUtility();
    }
    ...
    private static function ctx(array $over = []): BlutvAuthenticationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BlutvAuthenticationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BlutvAuthenticationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BlutvAuthenticationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BlutvAuthenticationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BlutvAuthenticationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BlutvAuthenticationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BlutvAuthenticationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BlutvAuthenticationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BlutvAuthenticationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BlutvAuthenticationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BlutvAuthenticationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BlutvAuthenticationResponse([]),
            'result' => new BlutvAuthenticationResult([]),
        ]);
        [, $err] = BlutvAuthenticationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BlutvAuthenticationSpec([]),
            'response' => null,
            'result' => new BlutvAuthenticationResult([]),
        ]);
        [, $err] = BlutvAuthenticationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BlutvAuthenticationSpec(['step' => 's']),
            'response' => new BlutvAuthenticationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BlutvAuthenticationResult([]),
        ]);
        [, $err] = BlutvAuthenticationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BlutvAuthenticationSpec(['step' => 's']),
            'response' => new BlutvAuthenticationResponse(self::resp(200, ['v' => 1])),
            'result' => new BlutvAuthenticationResult([]),
        ]);
        [, $err] = BlutvAuthenticationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BlutvAuthenticationSpec(['step' => 's']),
            'response' => new BlutvAuthenticationResponse(self::resp(200, ['v' => 2])),
            'result' => new BlutvAuthenticationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BlutvAuthenticationMakeResponse::call($ctx);
    ...
    {
        $preset = new BlutvAuthenticationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BlutvAuthenticationSpec([]),
            'response' => new BlutvAuthenticationResponse([]),
            'result' => new BlutvAuthenticationResult([]),
        ...
        [$got, $err] = BlutvAuthenticationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BlutvAuthenticationResult([])]);
        [, $err] = BlutvAuthenticationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BlutvAuthenticationSpec([]), 'result' => null]);
        [, $err] = BlutvAuthenticationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BlutvAuthenticationSpec(['step' => 's']),
            'result' => new BlutvAuthenticationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BlutvAuthenticationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BlutvAuthenticationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BlutvAuthenticationSpec(['step' => 's']),
            'result' => new BlutvAuthenticationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BlutvAuthenticationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BlutvAuthenticationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BlutvAuthenticationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BlutvAuthenticationSpec([]),
            'result' => new BlutvAuthenticationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BlutvAuthenticationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BlutvAuthenticationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BlutvAuthenticationError('boom', 'boom');
        $utility->fetcher = function (BlutvAuthenticationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BlutvAuthenticationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BlutvAuthenticationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BlutvAuthenticationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BlutvAuthenticationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BlutvAuthenticationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BlutvAuthenticationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BlutvAuthenticationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BlutvAuthenticationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BlutvAuthenticationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BlutvAuthenticationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BlutvAuthenticationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BlutvAuthenticationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BlutvAuthenticationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BlutvAuthenticationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BlutvAuthenticationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BlutvAuthenticationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BlutvAuthenticationResult(['ok' => false])]);
        $this->expectException(BlutvAuthenticationError::class);
        BlutvAuthenticationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BlutvAuthenticationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BlutvAuthenticationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BlutvAuthenticationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BlutvAuthenticationBaseFeature();
        $b = new BlutvAuthenticationBaseFeature();
        BlutvAuthenticationFeatureAdd::call($ctx, $a);
        BlutvAuthenticationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BlutvAuthenticationBaseFeature
    {
        $f = new BlutvAuthenticationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BlutvAuthenticationFeatureAdd::call($ctx, self::named_feature('a'));
        BlutvAuthenticationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        BlutvAuthenticationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        BlutvAuthenticationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        BlutvAuthenticationFeatureAdd::call($ctx, $replace);
        ...
        BlutvAuthenticationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BlutvAuthenticationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BlutvAuthenticationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BlutvAuthenticationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BlutvAuthenticationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BlutvAuthenticationResponse(['headers' => null]),
            'result' => new BlutvAuthenticationResult([]),
        ]);
        BlutvAuthenticationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BlutvAuthenticationResultBody::call($ctx);
