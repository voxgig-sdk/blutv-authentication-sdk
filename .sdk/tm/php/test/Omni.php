...
//    only - a typed BlutvAuthenticationContext there would make every ctx
...
final class BlutvAuthenticationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BlutvAuthenticationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BlutvAuthenticationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BlutvAuthenticationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BlutvAuthenticationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BlutvAuthenticationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BlutvAuthenticationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BlutvAuthenticationError(is_string($code) ? $code : '', $msg);
    }
