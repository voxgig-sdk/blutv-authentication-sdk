# BlutvAuthentication SDK test runner
...
class BlutvAuthenticationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BlutvAuthenticationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BlutvAuthenticationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BlutvAuthenticationTestRunner.getenv("BLUTV_AUTHENTICATION_TEST_LIVE")
        override = BlutvAuthenticationTestRunner.getenv("BLUTV_AUTHENTICATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BlutvAuthenticationTestRunner.getenv(key)
        ...
        explain = BlutvAuthenticationTestRunner.getenv("BLUTV_AUTHENTICATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BlutvAuthenticationTestRunner._test_control is not None:
            return BlutvAuthenticationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BlutvAuthenticationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BlutvAuthenticationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BlutvAuthenticationTestRunner.load_test_control()
...
def load_env_local():
    BlutvAuthenticationTestRunner.load_env_local()


def env_override(m):
    return BlutvAuthenticationTestRunner.env_override(m)


def entity_data(v):
    return BlutvAuthenticationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BlutvAuthenticationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BlutvAuthenticationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BlutvAuthenticationTestRunner.load_test_control()


def live_client_options():
    return BlutvAuthenticationTestRunner.live_client_options()


def live_delay_ms():
    return BlutvAuthenticationTestRunner.live_delay_ms()
