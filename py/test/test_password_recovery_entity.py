...
class TestPasswordRecoveryEntity:

    def test_should_create_instance(self):
        testsdk = BlutvAuthenticationSDK.test(None, None)
        ent = testsdk.PasswordRecovery(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _password_recovery_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "password_recovery." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        password_recovery_ref01_ent = client.PasswordRecovery(None)
...
def _password_recovery_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/password_recovery/PasswordRecoveryTestData.json")
