...
class TestSocialLoginEntity:

    def test_should_create_instance(self):
        testsdk = BlutvAuthenticationSDK.test(None, None)
        ent = testsdk.SocialLogin(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _social_login_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "social_login." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUTV_AUTHENTICATION_TEST_SOCIAL_LOGIN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        social_login_ref01_ent = client.SocialLogin(None)
        social_login_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.social_login"), "social_login_ref01"))

        social_login_ref01_data = helpers.to_map(runner.entity_data(social_login_ref01_ent.create(social_login_ref01_data, None)))
        assert social_login_ref01_data is not None
        assert social_login_ref01_data["id"] is not None



def _social_login_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/social_login/SocialLoginTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUTV_AUTHENTICATION_TEST_SOCIAL_LOGIN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUTV_AUTHENTICATION_TEST_SOCIAL_LOGIN_ENTID"))
