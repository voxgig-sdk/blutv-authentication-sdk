...
require_relative 'BlutvAuthentication_types'
...
class BlutvAuthenticationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BlutvAuthenticationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BlutvAuthenticationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BlutvAuthenticationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BlutvAuthenticationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BlutvAuthenticationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BlutvAuthenticationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BlutvAuthenticationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BlutvAuthenticationError.new(
        "#{op}_allow",
        "BlutvAuthenticationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue BlutvAuthenticationError => err
    ...
    if fetched.is_a?(Hash)
      status = BlutvAuthenticationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BlutvAuthenticationError.new(
        "graphql_error", "BlutvAuthenticationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BlutvAuthenticationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
