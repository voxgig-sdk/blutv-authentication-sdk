...
class PasswordRecoveryEntity
  ...
  def make
    opts = @_entopts.dup
    PasswordRecoveryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BlutvAuthenticationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PasswordRecovery, Hash] the current PasswordRecovery data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BlutvAuthenticationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PasswordRecovery fields)
  def match_get
  ...
  #
  # @param reqdata [PasswordRecoveryCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [PasswordRecovery, Hash] the created PasswordRecovery; raises BlutvAuthenticationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = BlutvAuthenticationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
