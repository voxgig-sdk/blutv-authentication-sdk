...
The Ruby SDK for the BlutvAuthentication API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.Login(nil).create({ "name" => "Example" }, nil)
...
result, err = client.BlutvAuthentication(nil).load(
  { "id" => "test01" }, nil
)
