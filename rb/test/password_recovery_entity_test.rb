...
class PasswordRecoveryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = password_recovery_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "password_recovery." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    password_recovery_ref01_ent = client.PasswordRecovery(nil)
...
def password_recovery_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "password_recovery", "PasswordRecoveryTestData.json")
