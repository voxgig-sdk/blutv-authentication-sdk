...
class SocialLoginEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUTV_AUTHENTICATION_TEST_SOCIAL_LOGIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    social_login_ref01_data = Helpers.to_map(social_login_ref01_data_result.respond_to?(:data_get) ? social_login_ref01_data_result.data_get : social_login_ref01_data_result)
    assert !social_login_ref01_data.nil?
    assert !social_login_ref01_data["id"].nil?
...
def social_login_basic_setup(extra)
  ...
  entid_env_raw = ENV["BLUTV_AUTHENTICATION_TEST_SOCIAL_LOGIN_ENTID"]
