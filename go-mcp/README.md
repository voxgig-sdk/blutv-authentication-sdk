# blutv-authentication-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BlutvAuthentication SDK as
two agent tools — `blutv-authentication_list` and `blutv-authentication_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/blutv-authentication-mcp)
make build

# 2. Provide credentials via the environment
export BLUTV_AUTHENTICATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user blutv-authentication \
  -- /absolute/path/to/blutv-authentication-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./blutv-authentication-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// blutv-authentication_list: first page of records
{ "entity": "login" }
{ "entity": "login", "query": { } }

// blutv-authentication_load: one record by id
{ "entity": "login", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/blutv-authentication-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user blutv-authentication \
     -- "$PWD"/dist/*/blutv-authentication-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `blutv-authentication_list` and `blutv-authentication_load` tools now appear
   in new sessions. Ask the agent to *"list login using blutv-authentication"*
   and it calls `blutv-authentication_list` with `{"entity":"login"}`.
...
```sh
export BLUTV_AUTHENTICATION_APIKEY=sk_live_xxx            # API key
export BLUTV_AUTHENTICATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `blutv-authentication_list` tool
...
### Call the `blutv-authentication_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `blutv-authentication_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `blutv-authentication_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BLUTV_AUTHENTICATION_APIKEY` | API key sent with every request. |
| `BLUTV_AUTHENTICATION_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

login | password_recovery | register | social_login

### Smoke test via HTTP (raw JSON-RPC)

```sh
./blutv-authentication-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"blutv-authentication_load","arguments":{"entity":"login","query":{"id":1}}}}'
```
