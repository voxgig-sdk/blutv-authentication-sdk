...
type Args struct {
	Entity string         `json:"entity" jsonschema:"login | password_recovery | register | social_login"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.BlutvAuthenticationSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "blutv-authentication_list",
		Description: "List records from BlutvAuthentication. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "blutv-authentication_load",
		Description: "Load a single record from BlutvAuthentication. " +
...
func runOp(client *sdk.BlutvAuthenticationSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.BlutvAuthenticationSDK, name string) (sdk.BlutvAuthenticationEntity, error) {
	switch strings.ToLower(name) {
	case "login":
		return client.Login(nil), nil
	case "password_recovery":
		return client.PasswordRecovery(nil), nil
	case "register":
		return client.Register(nil), nil
	case "social_login":
		return client.SocialLogin(nil), nil
